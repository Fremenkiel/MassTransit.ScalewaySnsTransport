...
/// that the exchanges, queues, and bindings for the model are properly configured in ScalewaySns.
/// </summary>
public class ConfigureScalewaySnsTopologyFilter<TSettings> :
    ...
    where TSettings : class
{
    ...
    public ConfigureScalewaySnsTopologyFilter(TSettings settings, BrokerTopology brokerTopology, SqsReceiveEndpointContext context = null)
    {
        _settings = settings;
    ...
    public async Task<OneTimeContext<ConfigureTopologyContext<TSettings>>> Configure(ClientContext context)
    {
        return await context.OneTimeSetup<ConfigureTopologyContext<TSettings>>(() =>
        {
            ...
            if (_context != null && AnyAutoDelete())
                _context.AddSendAgent(new RemoveScalewaySnsTopologyAgent(context, _brokerTopology));
