namespace MassTransit
{
    ...
    public readonly struct ScalewaySnsEndpointAddress
    {
        ...
        public ScalewaySnsEndpointAddress(Uri hostAddress, Uri address, AddressType type = AddressType.Queue)
        {
            Scheme = default;
            Host = default;
            Scope = default;
            ...
            switch (scheme)
            {
                case ScalewaySnsHostAddress.ScalewaySnsScheme:
            ...
            if (Type == AddressType.Queue)
                ScalewaySnsEntityNameValidator.Validator.ThrowIfInvalidEntityName(Name);
            else
                ScalewaySnsTopicNameValidator.Validator.ThrowIfInvalidEntityName(Name);
        }

        public ScalewaySnsEndpointAddress(Uri hostAddress, string name, bool durable = true, bool autoDelete = false, AddressType type = AddressType.Queue)
        {
            ParseLeft(hostAddress, out Scheme, out Host, out Scope);
        ...
        public static implicit operator Uri(in ScalewaySnsEndpointAddress address)
        {
